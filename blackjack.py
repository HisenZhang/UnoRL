...
temp_env = rlcard.make('blackjack')
blackjack_num_actions = temp_env.num_actions
blackjack_state_shape = temp_env.state_shape
...
def train(num_players, training_agents):
    ...
    for _ in range(num_players-initial_agents):
        agents.append(RandomAgent(num_actions=blackjack_num_actions))
    ...
    for e in range(episode_count):
        ...
        if e % eval_freq == 0:
            ...
            for i in range(len(training_agents)):
                ...
                # plt.plot(m, performances[i])

                plt.plot(performances[i])
                plt.savefig('data22/{}.png'.format(e))
                print(" Write to \"data22/{}.png\"".format(e))
    return performances


"""# Agent Maker and Results"""

dqn_agent = DQNAgent(num_actions=blackjack_num_actions,
                     state_shape=blackjack_state_shape[0],
...
sarsa_agent = SARSAAgent(dr=GAMMA,
                         lr=ALPHA,
                         mlp_layers=[64, 64],
                         num_actions=blackjack_num_actions,
                         state_shape=blackjack_state_shape[0],
                         epsilon_decay_steps=EPS_STEP)
m = None
